...
7、推送本地文件。首先打开git bash（管理员权限），目录设置为目标文件夹。输入 `git init` ，初始化成功后，输入 `git add .`，无报错后，输入 `git commit -m "首次部署博客"`（这个文字可能也会显示在仓库中）。无报错后，输入 `git push -u origin main`（默认仓库分支要和github上一致）。推送过程中会弹出登录认证，填写令牌内容即可。若最后显示done等信息表明推送成功。此时刷新github页面会看到推送的文件！

8、处理 `git push`推送不成功的问题。这一步报错绝大概率是网络设置问题。我尝试多种办法，解决思路是：首先测试能否访问 `ping github.com`；其次关闭校园网，用手机热点连接；然后关闭本地防火墙；大概率是校园网+防火墙的问题。能不用外挂的时候尽量也不要打开。
